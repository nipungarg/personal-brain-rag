...
def _retrieve_results_to_context(retrieved: list[tuple[float, str, str]]) -> list[dict]:
    """Convert (score, chunk_id, preview) to dicts expected by build_prompt."""
    context = []
    for score, chunk_id, preview in retrieved:
        ...
        context.append({"source": source, "chunk_index": idx, "id": chunk_id, "text": preview})
    return context
...
if __name__ == "__main__":
    ...
    out = generate_response(q, retrieved)
    print("\nANSWER:", out["answer"])
